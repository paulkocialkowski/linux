				   ...
				   unsigned long bytesused)
{
	...
	pr_debug("rc-stats: size: %lu B, target: %lu B, error: %ld B, drift: %ld pc\n",
		 bytesused, rc->size, size_error, size_drift);
	pr_debug("rc-stats: bitrate: %lu b/s, target: %u b/s, error: %ld b/s, drift: %ld pc\n",
...
complete:
	...
	if (9 * stats->size_total > 10 * stats->count * target_nominal) {
		pr_debug("rc-cbr: overshoot, reducing target size\n");
		target = 8 * target / 10;
	}
...
static int cbr_qp_initial(struct v4l2_h264_enc_rc *rc, unsigned int *qp)
{
	...
	pr_debug("rc-cbr: initial bits/mb: %lu\n", size_mb);
	...
	*qp = qp_initial;
	...
	pr_debug("rc-cbr: initial qp: %u\n", *qp);

	return 0;
}

/*
 * Size ratio thresholds (base 1000) for QP diff starting at 0. QP diff (index)
 * is an increase (positive) for ratios < 1 (smaller target size) and a decrease
 * (negative) otherwise (larger target size).
 *
 * This allows predicting the QP diff from an observed previous frame with known
 * size and QP using the current target size.
 */

static unsigned long cbr_ratio_qp_diff_steps_inc[] = {
	953, 866, 787, 715, 650, 590, 537, 488, 443, 403, 366, 332, 302, 274,
	249, 227, 206, 187, 170, 154, 140, 127, 116, 105, 96, 87, 79, 72, 65,
	59, 54, 49, 44, 40, 36
};

static unsigned long cbr_ratio_qp_diff_steps_dec[] = {
	1048, 1154, 1270, 1397, 1537, 1692, 1862, 2048, 2254, 2480, 2729, 3003,
	3305, 3636, 4001, 4403, 4845, 5331, 5867, 6455, 7103, 7816, 8601, 9464,
	10414, 11459, 12609, 13875, 15268, 16800, 18486, 20341, 22383, 24630,
	27101
};
...
static int cbr_ratio_qp_diff(unsigned long ratio, unsigned char slice_type)
{
	...
	/* Increase QP for smaller sizes and decrease for bigger ones. */
	if (ratio < 1000) {
		steps = cbr_ratio_qp_diff_steps_inc;
		count = ARRAY_SIZE(cbr_ratio_qp_diff_steps_inc);
		sign = 1;
	} else {
		steps = cbr_ratio_qp_diff_steps_dec;
		count = ARRAY_SIZE(cbr_ratio_qp_diff_steps_dec);
		sign = -1;
	}

	/*
	 * The static penalty values make sense for intra frames but are
	 * generally too aggressive for inter frames.
	 */
	...
	for (i = 0; i < count; i++) {
		...
		if ((sign > 0 && ratio > step) || (sign < 0 && ratio < step))
			return (int)i * sign * multiplier / divider;
	}
...
static int cbr_estimate(struct v4l2_h264_enc_rc *rc, unsigned int *qp)
{
	...
	pr_debug("rc-cbr: frame type: %c, target size: %lu\n",
		 v4l2_h264_slice_type_char(encode->slice_type), rc->size);
	...
	for (i = 0; i < count; i++) {
		...
		ratio = 1000 * rc->size / type->size[index];
		ratio_qp_diff = cbr_ratio_qp_diff(ratio, encode->slice_type);

		if ((int)type->qp[index] + ratio_qp_diff < 0)
			ratio_qp[i] = 0;
		else if (type->qp[index] + ratio_qp_diff > 51)
			ratio_qp[i] = 51;
		else
			ratio_qp[i] = type->qp[index] + ratio_qp_diff;

		diff = abs((long)rc->size - type->size[index]);
		pr_debug("rc-cbr: - backlog %d - size: %lu (diff: %lu), qp: %u, penalty %u, ratio qp: %u\n",
	...
	for (i = 0; i < count; i++) {
		...
		pr_debug("rc-cbr: - backlog %d - weight: %u\n", -(i + 1),
			 weight[i]);
	}

	*qp = total / weight_total;

	pr_debug("cbr: weighted qp: %u\n", *qp);
