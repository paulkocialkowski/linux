...
int hantro_vc8000e_h264_enc_run(struct hantro_ctx *ctx)
{
...
#ifdef DEBUG
	printk(KERN_ERR "VC8000E minor: %#x, major: %#x, id: %#x\n",
	       regs->swreg0.minor_number, regs->swreg0.major_number,
	       regs->swreg0.product_id);
#endif
...
#ifdef DEBUG
	regs->swreg199.hash_type = HANTRO_VC8000E_SWREG199_HASH_TYPE_CRC32;
#else
...
#ifdef DEBUG
	unsigned int i;
	u32 *pointer = (u32 *)regs;

	for (i = 0; i < sizeof(*regs) / 4; i++) {
		printk(KERN_ERR "hwtrace-write: %#x %#x\n", 4 * i, *pointer);
		pointer++;
	}
#endif
	...
	/* TODO; return buffer on error, maybe with common helper. */
}
...
#ifdef DEBUG
	hantro_vc8000e_swreg_read(vpu, regs, swreg200);
	printk(KERN_ERR "%s: hash %#x\n", __func__, regs->swreg200.hash_val);
#endif
